...
Architecture (NEW):
    [audio?] → [stt] → [supervisor]
              ...
              ┌─────────────┼──────────────────┐
              ▼             ▼                  ▼
        [soil_crop]    [weather]          [market]
              │             │                  │
              ▼             ▼                  ▼
        [deep_analysis_orchestrator]  ← ALL intents flow here
              │    (parallel: RAG + web + weather + market)
              ▼
        [synthesis_node]  ← combines everything into ONE rich response
              │
              ▼
        [response_formatter]
              │
...
2. Identify ONE primary intent: soil_crop | weather | market | rag_agronomy | general
...
def get_llm(temperature: float = 0.3, model: str = "openai/gpt-oss-120b") -> ChatGroq:
    ...
    if owm_key:
        try:
            ...
            if resp.status_code == 200:
                ...
                print(f"🌤️ Weather: {weather_data['temp']}°C, {weather_data['desc']} in {location}")
        except Exception as e:
...
def synthesis_node(state: AgroBotState) -> AgroBotState:
    ...
    # Use higher temperature for more natural, helpful language
...
def build_agrobot_graph() -> StateGraph:
    ...
    graph.add_node("data_gatherer",      data_gatherer_node)   # NEW
    graph.add_node("synthesis",          synthesis_node)       # NEW (replaces 5 specialist nodes)
    ...
    graph.add_edge("supervisor",     "data_gatherer")   # always gather data
    graph.add_edge("data_gatherer",  "synthesis")       # always synthesize
...
if __name__ == "__main__":
    ...
    print("   Type 'exit' to quit | '--audio' for voice output")
    print("=" * 70)
